# Memory model — where each agent's knowledge lives  (EXAMPLE — fictional)

Part of the control plane. Every agent reads this; **Jarvis** owns it. The full rationale
...
- **Jarvis reads:** `_control/` **+** the full `MEMORY.md` index — routing needs to know
...
- An agent **writes, updates and deletes only within its own namespace**, and may **propose**
  a `_shared/` addition.
- Only **Jarvis** — or an explicit fleet-wide instruction from Jane Doe ("from now on,
...
> **Budget: 20 KB total.** Full is full — **adding means demoting** something into an
> agent namespace.
