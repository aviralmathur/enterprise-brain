# How we work — fleet-wide rules  (EXAMPLE — fictional)
...
- **Send only on Jane Doe's explicit, per-item instruction.** No blanket auto-send.
...
- Each agent signs as the identity its `roster.md` row names. **Identity never crosses
  ...
  signs as Jane Doe, no agent line, no internal links.

## Discipline

- **No date, scope, price or effort estimate from any agent.** Those are Jane Doe's.
...
- **Messages are short.** A tactical ask is 1–3 lines, one question mark. Detail goes to
  Jane Doe or the tracker, not the recipient.
...
- Write only your own namespace; propose fleet facts. Only Jarvis writes `_shared/`.
